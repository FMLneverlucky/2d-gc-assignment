...
bool CGUI_Scene2D::Init(void)
{
	...
	ImGui_ImplGlfw_InitForOpenGL(CSettings::GetInstance()->pWindow, true); //useful when there is multiple imGui window/ imGui can customize to different window
...
bool CGUI_Scene2D::Update(const double dElapsedTime)
{
	...
	ImGui_ImplOpenGL3_NewFrame(); //tell is openglframe
	ImGui_ImplGlfw_NewFrame();	//tell is glfwframe
	ImGui::NewFrame();	//imgui frame
	...
	// Create an invisible window which covers the entire OpenGL window(important)
	...
	//default: positioned by row, definie desired position
	...
	ImGui::Image((void*)(intptr_t)cInventoryItem->GetTextureID(),
		ImVec2(cInventoryItem->vec2Size.x * relativeScale_x, cInventoryItem->vec2Size.y * relativeScale_y),
		ImVec2(0, 1), ImVec2(1, 0));
	ImGui::SameLine(); //will go to next line by default, so must define it to be on same line
	ImGui::SetWindowFontScale(1.5f * relativeScale_y);
	...
	ImGui::End(); //programme will crash if one push/pop is missing
